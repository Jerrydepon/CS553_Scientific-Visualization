...
const char *WINDOWTITLE = { "Project #2 -- Chih-Hsiang Wang" };
...
Display()
{
		...
		for (int j = 0; j < NY; j++)
		{
			...
			for (int k = 0; k < NZ; k++)
			{
				if (Nodes[0][0][k].z < ZLowHigh[0] || Nodes[0][0][k].z > ZLowHigh[1])
					continue;
				//check the temperature slider
				if (Nodes[i][j][k].t < TLowHigh[0] || Nodes[i][j][k].t > TLowHigh[1])
					continue;
	...
	glLoadIdentity();
	glColor3f(1., 1., 1.);
	DoRasterString(5., 5., 0., "Project #2 -- Chih-Hsiang Wang");
	...
	for (int i = 0; i < NX; i++, x += delx)
	{
		...
		for (int j = 0; j < NY; j++, y += dely)
		{
			...
			for (int k = 0; k < NZ; k++, z += delz)
			{
				...
				//r = g = b gives gray
				Nodes[i][j][k].rgb[0] = Nodes[i][j][k].rgb[1] = Nodes[i][j][k].rgb[2] = gray;
			}
		}
	}
