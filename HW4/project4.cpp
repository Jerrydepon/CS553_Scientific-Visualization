...
const char *WINDOWTITLE = { "Project #4 -- Chih-Hsiang Wang" };
...
const float KMIN = { 0.00 };
const float KMAX = { 1.00 };
...
Display()
{
	...
	for (int i = 0; i < 68; i++)
	{
		...
		for (int j = 0; j < Npts[i]; j++)
		{
			...
			{
				xp = x / sqrtf(SQR(x) + SQR(Kvalue));
				yp = y / sqrtf(SQR(y) + SQR(Kvalue));
			}
			else
			{
				xp = x / (r + Kvalue);
			...
			if (TintersectionsOff == GLUIFALSE)
			{
				glVertex2f(xp, yp);
			}
			else
	...
	glLoadIdentity();
	glColor3f(1., 1., 1.);
	DoRasterString(5., 5., 0., "Project #4 -- Chih-Hsiang Wang");
...
InitGlui()
{
	...
	Glui->add_checkbox("Solve T-intersection", &TintersectionsOff);
	Glui->add_checkbox("Polar Hyperbolic", &polarOn);

	GLUI_Rollout * rollout = Glui->add_rollout("Range Sliders", true);
	//TSlider = Glui->add_slider_to_panel(rollout, true, GLUI_HSLIDER_FLOAT, TLowHigh, T, (GLUI_Update_CB)Sliders);
	//TSlider->set_float_limits(TLowHigh[0], TLowHigh[1]);
	//TSlider->set_slider_val(TLowHigh[0], TLowHigh[1]);
	//TSlider->set_w(SLIDERWIDTH);
	//sprintf(str, TFORMAT, TLowHigh[0], TLowHigh[1]);
	//TLabel = Glui->add_statictext_to_panel(rollout, str);
	...
	Glui->add_separator();
	KSlider = Glui->add_slider(false, GLUI_HSLIDER_FLOAT, &Kvalue, 0, (GLUI_Update_CB)Sliders);
	sprintf(str, KFORMAT, Kvalue, Kvalue);
...
InitGraphics()
{
	//FILE *fp = fopen("C:/Users/jerry/Desktop/proj04.dat", "r");
